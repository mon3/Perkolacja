...
if __name__ == "__main__":
    ...
    with h5py.File("target.hdf5") as f:
        ...
        if 1000 in listy_S.keys():
           ...
           # K_length = np.zeros((n,N))
            ...
            for i in range(n):
              ...
              #  print "K[i] = ", K[i]
                ...
                # daje posortowane klastry
                #number_clusters_per_size_array = K[i]
              #  number_clusters_per_size_array[i] = clusters(K,N) #zapisac do pliku
               # for j in range(N):
                   # K_length[i] = (number_clusters_per_size_array ==(i+1)).sum()
            # for l in range(n):
            #     for j in range(N):
            #         K_length[j] = (K[l]==(j+1)).sum()
            #         klasterki[j] = K_length[j]
            #         print "K_length[j] = ", K_length[j]
          ...
          #  plt.show()    
         #   # mean_ncluster_sizes = K_append[50]

            

         #    mean_ncluster_sizes = np.mean(K_append[...], axis = 0)
         #    #mean_ncluster_sizes = np.sum(number_clusters_per_size_array[...], axis=0)
         #    print "mean_ncluster_sizes = ", mean_ncluster_sizes
         #    mean_ncluster_sizes = mean_ncluster_sizes[mean_ncluster_sizes>0]
         #    suma = np.sum(mean_ncluster_sizes)
         #    print "suma= ", suma
         #    x = (1 + np.arange(len(mean_ncluster_sizes)))
         #    y = (mean_ncluster_sizes)

         #   #  mean_ncluster_sizes = np.sum(number_clusters_per_size_array[...], axis=0) # może 1?
         #   # # print mean_ncluster_sizes
         #   #  #print "max" , max(mean_ncluster_sizes)
         #   #  mean_ncluster_sizes = mean_ncluster_sizes[mean_ncluster_sizes > 0]
         #   #  suma = np.sum(mean_ncluster_sizes)
         #   # # print(mean_ncluster_sizes)
         #   #  x = (1 + np.arange(len(mean_ncluster_sizes)))
         #   #  y = np.log(mean_ncluster_sizes)

         #    # warunek
         #    granica = x < 50
         #    x = 1+np.log(x[granica]) # specjalnie, zeby na x-ach sie zaczynalo od 1, a nie od 0
         #    y = np.log(y[granica])
         ...
         #    plt.plot(x, y,'o', label = 'symulacja')
