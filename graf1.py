    ...
    for i in range(N):
        ...
        for j in range(i+1, N):
            if ((adj_matrix[i][j] == 1) & (K[j] == 0)):
                K[j] = K[i]
            elif ((adj_matrix[i][j] == 1) & (K[j] != 0) & (changed == False) & (K[i]!=K[j])):
                changed = True
                value_for_change = K[j] # K[i] zamiast K[j]
                index = j
                old_value = K[i] #zamiast K[i] daje K[j]
                K[i] = value_for_change #zamiast K[i] daję K[j]
                for l in range(index): #zmieniam granice -> musi sprawdzic wszystkie wezly
                    if (K[l] == old_value):
